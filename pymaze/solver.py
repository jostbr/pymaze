...
class BreadthFirst(Solver):
    ...
    def solve(self):
        ...
        while True:  # Loop until return statement is encountered
            ...
            while (
                current_level
            ):  # While still cells left to search on current level
                ...
                if (
                    ...
                    if not self.quiet_mode:
                        print(
                            "Number of moves performed: {}".format(len(path))
                        )
                        print(
...
class BiDirectional(Solver):
    ...
    def solve(self):
        ...
        (
            k_curr,
            l_curr,
        ) = self.maze.entry_coor  # Where to start the first search
        (
            p_curr,
            q_curr,
        ) = self.maze.exit_coor  # Where to start the second search
        ...
        while True:  # Loop until return statement is encountered
            ...
            real_neighbours_kl = [
                ...
                if not grid[k_curr][l_curr].is_walls_between(
            ...
            real_neighbours_pq = [
                ...
                if not grid[p_curr][q_curr].is_walls_between(
                    grid[neigh[0]][neigh[1]]
                )
            ]
            neighbours_pq = [
            ...
            if any(
                (
                    True
                    for n_kl in real_neighbours_kl
                    if (n_kl, False) in path_pq
                )
            ):
                path_kl.append(((k_curr, l_curr), False))
                path = [
                    p_el
                    for p_tuple in zip(path_kl, path_pq)
                    for p_el in p_tuple
                ]  # Zip paths
                if not self.quiet_mode:
            ...
            # Exit loop and return path if any opf the pq neighbours are in path_kl.
                ...
                (
                    True
                    for n_pq in real_neighbours_pq
                    if (n_pq, False) in path_kl
                )
            ):
...
class DepthFirstBacktracker(Solver):
    ...
    def solve(self):
        ...
        self.maze.grid[k_curr][
            l_curr
        ].visited = True  # Set initial cell to visited
        ...
        while (
            ...
            if (
                ...
                self.maze.grid[k_next][
                    l_next
                ].visited = True  # Move to that neighbour
                k_curr = k_next
                l_curr = l_next
